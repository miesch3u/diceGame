...
import {initRules, getRules} from "./model/rules.js";
import {getTopScores} from "./model/highScore.js";
import {initOption, getElement, getValue, getName, setValue} from "./model/options.js";
...
window.onload = () => {
    ...
    const textRules = document.createElement("p");
    textRules.innerHTML = getRules(rules);
    div.appendChild(textRules);

    let textOptions = document.createElement("b")
    textOptions.innerHTML = getName(option1)
    div.appendChild(textOptions);

    let checkboxOption1 = getElement(option1);
    checkboxOption1.onclick = () => setValue(option1, checkboxOption1.checked);
    checkboxOption1.type = 'checkbox'
    div.appendChild(checkboxOption1);
}
...
var rules = initRules()
var option1 = initOption("Changer la condition de gain de score à sommeDés = 12", document.createElement("input"), false)

const saveScore = () => {
    save(joueur)
}

const updateScore = (de1, de2, nbRolls) => {
    if(getValue(option1)){
        if ((de1.valeur + de2.valeur) == 12) joueur.score += 10;
    } else {
